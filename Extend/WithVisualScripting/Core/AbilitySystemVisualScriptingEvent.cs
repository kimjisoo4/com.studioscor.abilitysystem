#if SCOR_ENABLE_VISUALSCRIPTING


namespace StudioScor.AbilitySystem.VisualScripting
{
    public static class AbilitySystemWithVisualScriptingEvent
    {
        public const string ABILITYSYSTEM_GRANT_ABILITY = "AbilitySystem_GrantAbility";
        public const string ABILITYSYSTEM_REMOVE_ABILITY = "AbilitySystem_RemoveAbility";
        public const string ABILITYSYSTEM_ACTIVE_ABILITY = "AbilitySystem_ActiveAbility";
        public const string ABILITYSYSTEM_FINISH_ABILITY = "AbilitySystem_FinishAbility";

        public const string ABILITYSPEC_ACTIVATED_ABILITY = "AbilitySpec_ActivatedAbility";
        public const string ABILITYSPEC_CANCELED_ABILITY = "AbilitySpec_CanceledAbility";
        public const string ABILITYSPEC_FINISHED_ABILITY = "AbilitySpec_FinishedAbility";
        public const string ABILITYSPEC_ENDED_ABILITY = "AbilitySpec_EndedAbility";
        public const string ABILITYSPEC_CHANGED_ABILITY_LEVEL = "AbilitySpec_ChangedAbilityLevel";

        public const string ABILITYSPEC_CAN_ACTIVE_ABILITY = "AbilitySpec_CanActiveAbility";
        public const string ABILITYSPEC_ENTER_ABILITY = "AbilitySpec_EnterAbility";
        public const string ABILITYSPEC_EXIT_ABILITY = "AbilitySpec_ExitAbility";
        public const string ABILITYSPEC_FINISH_ABILITY = "AbilitySpec_FInishAbility";
        public const string ABILITYSPEC_CANCEL_ABILITY_FROM_SOURCE = "AbilitySpec_CancelAbilityFromSource";
        public const string ABILITYSPEC_CANCEL_ABILITY = "AbilitySpec_CancelAbility";
        public const string ABILITYSPEC_CAN_RETRIGGER_ABILITY = "AbilitySpec_CanReTriggerAbility";
        public const string ABILITYSPEC_RETRIGGER_ABILITY = "AbilitySpec_ReTriggerAbility";
        public const string ABILITYSPEC_RELEASE_ABILITY = "AbilitySpec_ReleaseAbility";
        public const string ABILITYSPEC_GRANT_ABILITY = "AbilitySpec_GrantAbility";
        public const string ABILITYSPEC_REMOVE_ABILITY = "AbilitySpec_RemoveAbility";
        public const string ABILITYSPEC_OVERRIDE_ABILITY = "AbilitySpec_OverrideAbility";
        public const string ABILITYSPEC_CHANGE_ABILITY_LEVEL = "AbilitySpec_ChangeAbilityLevel";
    }
}

#endif